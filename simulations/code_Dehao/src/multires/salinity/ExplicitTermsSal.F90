    ...
    do ic=xstartr(3),xendr(3)
        ...
        do jc=xstartr(2),xendr(2)
            ...
            if(sidewall.eq.1) then
               if(ic.eq.1) then
                  dzzs=(sal(kc,jc,ip) - 1.0*sal(kc,jc,ic) + 0.0d0        )*udzrq
               elseif(ic.eq.nzmr) then
                  dzzs=(0.0d0         - 1.0*sal(kc,jc,ic) + sal(kc,jc,im))*udzrq
               else
            ...
            if(sidewall.eq.1) then
               if(jc.eq.1) then
                  dyys=(sal(kc,jp,ic) - 1.0*sal(kc,jc,ic) + 0.0d0        )*udyrq
               elseif(jc.eq.nymr) then
                  dyys=(0.0d0         - 1.0*sal(kc,jc,ic) + sal(kc,jm,ic))*udyrq
               else
    ...
    if (phasefield) then
        do ic=xstartr(3),xendr(3)
            ...
            do jc=xstartr(2),xendr(2)
                ...
                if(sidewall.eq.1) then
                   if(jc.eq.1) then
                      dpdsy = udyr**2*(phi(kc,jp,ic) - phi(kc,jc,ic))*(sal(kc,jp,ic) - sal(kc,jc,ic))
                   elseif(jc.eq.nymr) then
                      dpdsy = udyr**2*(phi(kc,jc,ic) - phi(kc,jm,ic))*(sal(kc,jc,ic) - sal(kc,jm,ic))
                   else
                      dpdsy = udyr**2*(phi(kc,jp,ic) - phi(kc,jm,ic))*(sal(kc,jp,ic) - sal(kc,jm,ic))
                   endif
                else
                      dpdsy = udyr**2*(phi(kc,jp,ic) - phi(kc,jm,ic))*(sal(kc,jp,ic) - sal(kc,jm,ic))
                endif
                ...
                if(sidewall.eq.1) then
                   if(ic.eq.1) then
                      dpdsz = udzr**2*(phi(kc,jc,ip) - phi(kc,jc,ic))*(sal(kc,jc,ip) - sal(kc,jc,ic))
                   elseif(ic.eq.nzmr) then
                      dpdsz = udzr**2*(phi(kc,jc,ic) - phi(kc,jc,im))*(sal(kc,jc,ic) - sal(kc,jc,im))
                   else
                      dpdsz = udzr**2*(phi(kc,jc,ip) - phi(kc,jc,im))*(sal(kc,jc,ip) - sal(kc,jc,im))
                   endif
                else
                   dpdsz = udzr**2*(phi(kc,jc,ip) - phi(kc,jc,im))*(sal(kc,jc,ip) - sal(kc,jc,im))
                endif
