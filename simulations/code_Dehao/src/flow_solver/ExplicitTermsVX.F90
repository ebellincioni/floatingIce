...
subroutine ExplicitTermsVX
    ...
    pf_eta = dtmax
    !pf_eta = 2.0*ren*(1.51044285*pf_eps)**2
        ...
        do jc=xstart(2),xend(2)
            ...
            do kc=2,nxm
                ...
                if(sidewall.eq.1) then
                  if(ic.eq.1) then
                      dzzvx=(0.0d0      -1.0*vx(kc,jc,ic)+vx(kc,jc,ip))*udzq    !freeslip BC
                  elseif(ic.eq.nzm) then
                      dzzvx=(vx(kc,jc,im)-1.0*vx(kc,jc,ic)+      0.0d0)*udzq
                  else
                      dzzvx=(vx(kc,jc,im)-2.0*vx(kc,jc,ic)+vx(kc,jc,ip))*udzq
                  endif
                else
                  dzzvx=(vx(kc,jc,im)-2.0*vx(kc,jc,ic)+vx(kc,jc,ip))*udzq
                endif
                ...
                if(sidewall.eq.1) then
                  if(jc.eq.1) then
                      dyyvx=(0.0d0      -3.0*vx(kc,jc,ic)+vx(kc,jp,ic))*udyq   !noslip BC
                  elseif(jc.eq.nym) then
                      dyyvx=(vx(kc,jm,ic)-3.0*vx(kc,jc,ic)+      0.0d0)*udyq
                  else
                      dyyvx=(vx(kc,jm,ic)-2.0*vx(kc,jc,ic)+vx(kc,jp,ic))*udyq
                  endif
                else
                  dyyvx=(vx(kc,jm,ic)-2.0*vx(kc,jc,ic)+vx(kc,jp,ic))*udyq
                endif
        ...
        do ic=xstart(3),xend(3)
            do jc=xstart(2),xend(2)
                do kc=2,nxm
                    ...
                    qcap(kc,jc,ic) = qcap(kc,jc,ic) + byct*abs(tempit-Td+ pf_denscoef*salit)**2.0*cos(theta/180.0*pi)! + byct*tempit*cos(theta/180.0*pi)
                end do
            end do
        end do
            ...
            do ic=xstart(3),xend(3)
                do jc=xstart(2),xend(2)
                    do kc=2,nxm
                        km=kc-1
                        salit =active_S*0.5d0*(salc(kc,jc,ic)+salc(km,jc,ic))
                        qcap(kc,jc,ic) = qcap(kc,jc,ic) - bycs*salit*cos(theta/180.0*pi)
                    end do
                end do
            end do
        endif
