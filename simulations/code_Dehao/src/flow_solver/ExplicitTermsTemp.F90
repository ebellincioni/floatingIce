...
!
      if(sidewall.eq.1) then
         if(ic.eq.1) then
            dzzt=(temp(kc,jc,ip)-1.0*temp(kc,jc,ic)+0.0d0         )*udzq
         elseif(ic.eq.nzm) then
            dzzt=(0.0d0         -1.0*temp(kc,jc,ic)+temp(kc,jc,im))*udzq
         else
...
!
      if(sidewall.eq.1) then
         if(jc.eq.1) then
            dyyt=(temp(kc,jp,ic)-1.0*temp(kc,jc,ic)+0.0d0         )*udyq
         elseif(jc.eq.nym) then
            dyyt=(0.0d0         -1.0*temp(kc,jc,ic)+temp(kc,jm,ic))*udyq
         else
        ...
        ! add these values to hro in the case when kc = 1
        hro(kc,jc,ic) = -(htx+hty+htz)+dyyt+dzzt
     !    hro(kc,jc,ic) = dyyt+dzzt

        ! now look at the intermediate values
        do kc=2,nxm-1
         km=kc-1
         kp=kc+1
         !
