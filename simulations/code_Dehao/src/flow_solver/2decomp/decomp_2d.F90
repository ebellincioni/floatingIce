...
#include "global.fh"
  ...
  TYPE, public :: DECOMP_INFO
     ...
     ! This is for the real datatype
     integer,dimension(:),allocatable::zcnts_xzr,ztypes_xzr
     integer,dimension(:),allocatable::xcnts_xzr,xtypes_xzr
#ifdef MPI3                               
! use MPI_ADDRESS_KIND for MPI_Neighbor_alltoallw call
     ...
     integer(kind=MPI_ADDRESS_KIND),dimension(:),allocatable::zdispls_xzr,xdispls_xzr
...
! use default integer for MPI_Alltoallw call
     ...
     integer,dimension(:),allocatable::zdispls_xzr,xdispls_xzr
#endif
!#endif
...
99  format('log',I2.2)
  ...
  subroutine decomp_info_init(nx,ny,nz,decomp)
    ...
    ! For real transpose
    allocate(decomp%xcnts_xzr(nproc),decomp%zcnts_xzr(nproc))
    allocate(decomp%xtypes_xzr(nproc),decomp%ztypes_xzr(nproc))
    allocate(decomp%xdispls_xzr(nproc))
    allocate(decomp%zdispls_xzr(nproc))
  ...
  subroutine decomp_info_finalize(decomp)
    ...
    ! For real transpose
    deallocate(decomp%xcnts_xzr,decomp%zcnts_xzr)
    deallocate(decomp%xtypes_xzr,decomp%ztypes_xzr)
    deallocate(decomp%xdispls_xzr)
    deallocate(decomp%zdispls_xzr)
#ifdef MPI3
  ...
  subroutine prepare_buffer(decomp)
    ...
    integer :: ierror
#ifdef MPI3
    integer,dimension(nproc) :: xranks1,zranks1
    ...
    ! For real transpose 
    decomp%xdispls_xzr(:)=0
    decomp%zdispls_xzr(:)=0
    decomp%xcnts_xzr(:)=0
    decomp%zcnts_xzr(:)=0
...
!JD no checks on x- or z-dimension, since we transform from z- into x-pencils, so these always overlap.
           if (decomp%zst(2).le.decomp%y1en(k) .and. &
             ...
             call MPI_Type_create_subarray(3,decomp%zsz, &
               (/decomp%zsz(1),subsize_y,decomp%z2dist(i)/), &
               (/0,offset_y,decomp%z2st(i)-decomp%zst(3)/), &
               MPI_ORDER_FORTRAN,real_type,decomp%ztypes_xzr(rank_z+1),ierror)
             call MPI_Type_commit(decomp%ztypes_xzr(rank_z+1),ierror)
...
!JD no checks on x- or z-dimension, since we transform from z- into x-pencils, so these always overlap.
           if (decomp%xst(2).le.decomp%y2en(i) .and. &
             ...
             call MPI_Type_create_subarray(3,decomp%xsz, &
               (/decomp%x1dist(k),subsize_y,decomp%xsz(3)/), &
               (/decomp%x1st(k)-decomp%xst(1),offset_y,0/), &
               MPI_ORDER_FORTRAN,real_type,decomp%xtypes_xzr(rank_x+1),ierror)
             call MPI_Type_commit(decomp%xtypes_xzr(rank_x+1),ierror)
...
!                   decomp%xst(3):decomp%xen(3)
    ...
    decomp%xranks=xranks1(1:index_dest)+1
    decomp%zranks=zranks1(1:index_src)+1

    call MPI_Dist_graph_create_adjacent(DECOMP_2D_COMM_CART_X, &
      index_src,zranks1(1:index_src),zweights(1:index_src), &
      index_dest,xranks1(1:index_dest),xweights(1:index_dest), &
