...
!
    ...
    if (sidewall.eq.0) then
      periodic_bc = (/.false., .true., .true./)
    else if (sidewall.eq.1) then
      periodic_bc = (/.false., .false., .false./)
    else
      periodic_bc = (/.false., .false., .true./)
    end if 
    write(*,*) 'Periodic conditions: ', periodic_bc(1:3)

    call decomp_2d_init(nxm,nym,nzm,nxmr,nymr,nzmr,prow,pcol,periodic_bc)
